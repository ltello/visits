...
describe TextFormatter do
  ...
  describe "#average_visits" do
    shared_examples "an average visits formatter" do
      let(:analizer) { Analizer.new(log) }
      subject(:formatter) { described_class.new(analizer) }

      it "returns a hash counting the average_visits" do
        expect(formatter.average_visits).to eq(expected)
      end
    end
    ...
    context "when there are no requests" do
      ...
      it_behaves_like "an average visits formatter"
    end

    context "when all requests hit the same endpoint" do
      let(:log) { "fixtures/single_endpoint.log" }
      let(:expected) { "/help_page/1 1 visit" }
